...
func updateModule(moduleToUpdate versionManager.ModuleVersion, project project.Project, sourceControl sourceControl, provider provider, existingPRs []p.PullRequest, commitMessage string) {
	...
	for _, pr := range existingPRs {
		...
		if !openPRAlreadyExists && strings.HasPrefix(pr.GetSourceBranchName(), dependencyBranchVersionPrefix) {
			if pr.GetState() == "OPEN" {
				log.Logger.Infof("There already is an open PR for: '%s'. The branch name is: %s.", title, pr.GetSourceBranchName())
				openPRAlreadyExists = true
			} else {
				log.Logger.Infof("There was a declined PR for: '%s'. The branch name is: %s.", title, pr.GetSourceBranchName())
				declinedPRAlreadyExists = true
			}
			continue
		}

		if pr.GetState() == "OPEN" && strings.HasPrefix(pr.GetSourceBranchName(), dependencyBranchPrefix) {
			if os.Getenv("DRY_RUN") == "1" {
				log.Logger.Infof("Running in DryRun mode. PR '%s' made for older version would be declined.", pr.GetTitle())
			} else {
				log.Logger.Infof("Declining PR '%s' made for older version.", pr.GetTitle())
				provider.DeclinePullRequest(project.Owner, project.Name, pr.GetId())
			}
		}
	}
...
func isBranchDead(branch string, existingPRs []p.PullRequest) bool {
	for _, pr := range existingPRs {
		if pr.GetState() == "OPEN" && branch == pr.GetSourceBranchName() {
			return false
		}
	}
	return true
